    ...
    execute: function () {
      ...
      _export("BallController", BallController = (_dec = ccclass('BallController'), _dec2 = property(Prefab), _dec(_class = (_class2 = class BallController extends Component {
          ...
          if (other && other.node && contact) {
            ...
            if (other.node.name == 'Bottom-floor') {
              ...
              if (isGoldenBall) {
                ...
                director.emit('subtract-score', 20); //director.emit('subtract-bateo', 1); // Golden ball touching floor reduces bateos more

                this.instantiateMiniLabel(contact, 'golden-floor');
              } else {
                // Emit both score events (for tracking total score) and bateo events (for bateos counter)
                director.emit('subtract-score', 10); //director.emit('subtract-bateo', 5); // Normal ball touching floor reduces bateos
        ...
        instantiateMiniLabel(contact, type) {
          ...
          const miniLabel = instantiate(this.miniLabelPrefab); // Get the MiniLabelCtrl component and set text/color

          const miniLabelCtrl = miniLabel.getComponent('MiniLabelCtrl');

          if (miniLabelCtrl) {
            if (type === '+') {
              miniLabelCtrl.setText('+5');
              miniLabelCtrl.setColor('green');
            } else if (type === '-') {
              miniLabelCtrl.setText('-10');
...
//# sourceMappingURL=a5b90f76f98593598478e5907ca23c06b4123016.js.map